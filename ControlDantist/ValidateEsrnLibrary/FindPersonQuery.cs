...
namespace ControlDantist.ValidateEsrnLibrary
{
    public class FindPersonQuery : IQuery
    {
        ...
        public FindPersonQuery(string nameTemptable)
        {
        ...
        public string Query()
        {
            string queryJoin = @"select  " + this.nameTemptable + ".id_договор, OUID,Tab1.Фамилия,Tab1.Имя,Tab1.Отчество,DOCUMENTSTYPE,DOCUMENTSERIES as 'СерияДокумента',DOCUMENTSNUMBER as 'НомерДокумента',ISSUEEXTENSIONSDATE as 'ДатаВыдачи',A_NAME as 'НаименованиеДокумента'," +
                                " A_ADRTITLE as 'Адрес',BIRTHDATE as 'ДатаРождения',A_SNILS,A_REGREGIONCODE from( " +
                               ...
                               " 'Справка о признании пострадавшим от политических репрессий','Удостоверение ветерана военной службы','Паспорт гражданина России') ) as Tab1 " +
                                 " inner join " + this.nameTemptable + " " +
                                   ...
                                   /*
                                   " and LOWER(RTRIM(LTRIM(Tab1.DOCUMENTSERIES))) = LOWER(RTRIM(LTRIM(" + this.nameTemptable + ".СерияДокумента))) " +
                                   " and LOWER(RTRIM(LTRIM(Tab1.DOCUMENTSNUMBER))) = LOWER(RTRIM(LTRIM(" + this.nameTemptable + ".НомерДокумента)))   " +
                                   " and CONVERT(char(10), CAST(LOWER(RTRIM(LTRIM(Tab1.ISSUEEXTENSIONSDATE))) as DATE), 104) = CONVERT(char(10), CAST(LOWER(RTRIM(LTRIM(" + this.nameTemptable + ".ДатаВыдачиДокумента))) as DATE), 104) "; */
