...
using ControlDantist.Bridges;
using ControlDantist.BalanceContract;
using ControlDantist.Reports;
using ControlDantist.ReportStatistic;
...
namespace ControlDantist
{
    public partial class FormInv : Form
    {
        ...
        private IEnumerable<ReportBalanceStatistic> ReestrStatisticAdd(List<ReportBalanceStatistic> listReestr, string hospital, string preferentCategory)
        {
            ...
            if(this.rbНаличиеАкта.Checked == true && this.rbОтсутствиеАкта.Checked  == false)
            {
                flagExecAct = true;
            }
            else
            ...
            IQuery qb = new QueryBalanceStatistic(hospital, preferentCategory, dateStart, dateEnd, flagExecAct);
            
            var queryStringTest = qb.Query();

            return GetStatistic.GenerateList(qb.Query());
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            // Поликлинника 
            ПоликлинникиИнн hospital = (ПоликлинникиИнн)this.cmbHjspital.SelectedItem;

            // Отсутствие акта выполненных работ.
            if(this.chbАкт.Checked == true && rbОтсутствиеАкта.Checked == true)
            {
                // SQL запрос к БД для поиска договоров прошедших проверку, но не имеющих акт выполненных работ.
                IQuery query = new QueryContractNoAct(dateStartReport, dateEndReport, hospital);

                // Получим таблицу с данными для отчета.
                DataTable tabContracts = ТаблицаБД.GetTableSQL(query.Query(), "ПроектыДоговоров");

                // Проверим есть лит таблица.
                if(tabContracts != null && tabContracts.Rows != null && tabContracts.Rows.Count > 0)
                {
                    IBridge<ReportBalance> bridge = new ConvertTableToReportBalance(tabContracts);
                    List<ReportBalance> listBalance = bridge.ConvertTo();

                    // Передадим в отчет.
                    IPrintReport print = new ReportBalanceContract(listBalance, dateStartReport, dateEndReport,this.cmbHjspital.Text);
                    print.Print();
