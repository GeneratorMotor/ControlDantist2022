...
{
    public class PersonWriteDBTemp : IValidBD<Т2Льготник>
    {
        private Т2Льготник person;

        private DContext dc;

        private bool flafValide = false;

        public PersonWriteDBTemp(DContext dc, Т2Льготник person)
        {
            this.person = person;
            this.dc = dc;
        }

        public Т2Льготник Get()
        {
            return this.person;
        }
        ...
        public bool Validate()
        {
            ...
            // Поиск льготника.
            var persons = dc.Т2Льготник.Where(w => w.Фамилия.Trim().ToLower() == person.Фамилия.Trim().ToLower() && w.Имя.Trim().ToLower() == person.Имя.ToLower().Trim() && w.Отчество.Trim().ToLower() == person.Отчество.Trim().ToLower() && w.ДатаРождения == person.ДатаРождения).OrderByDescending(w => w.id_льготник).FirstOrDefault();
