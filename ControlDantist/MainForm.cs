...
using ControlDantist.ClassErrors;
...
namespace ControlDantist
{
    public partial class MainForm : Form
    {
        ...
        private void договораПрошедшиеПроверкуToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            FormFIndVAlidTrueCOntracts fvc = new FormFIndVAlidTrueCOntracts();
            fvc.MdiParent = this;
            fvc.WindowState = FormWindowState.Maximized;
            fvc.Show();
        }
        ...
        {
            FormFindValidFalseContract fvc = new FormFindValidFalseContract();
            fvc.MdiParent = this;
            fvc.WindowState = FormWindowState.Maximized;
            fvc.Show();
        ...
        private void информацияПоБесплатномуЗубопротезированиюToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            // Форма для вывода дииалога отправки на печать.
            ...
            if (openFileDialog1.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                ...
                foreach (var unload in listUnload)
                {
                    ...
                    using (var dc = new DContext(ConnectDB.ConnectionString()))
                    {
                        ...
                        // Преобразуем id льготной категории к id на нашем сервере.
                        IReadRegistr<ТЛьготнаяКатегория> readRegistrLK = new ReadЛьготнаяКатегория(dc, льготнаяКатегория);
                        ...
                        if (тЛьготнаяКатегория != null)
                        {
                            тДоговор.id_льготнаяКатегория = тЛьготнаяКатегория.id_льготнойКатегории;
                        }
                        else
                        {
                            // Выберим Exception.
        ...
        private void залитьФайлыДляПроверкиToolStripMenuItem_Click(object sender, EventArgs e)
        {
            string path = @"E:\Новая папка (2)\ПРОВЕРКА";
            ...
            foreach (var item in dir.GetDirectories())
            {
                ...
                FileInfo[] filesName = item.GetFiles();
                ...
                foreach (var file in filesName)
                {
                    ...
                    {
                        string pathLog = @"F:\!\ErrorLogDantist.txt";
                        using (FileStream fs = File.OpenWrite(pathLog))
                        using (TextWriter writer = new StreamWriter(fs))
                        {
                            writer.WriteLine("Ошибка файла - " + file.FullName);
                        }
                    }
                    ...
                    using (var scope = new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = System.Transactions.IsolationLevel.Serializable }))
                    {
                        ...
                        try
                        {
                            ...
                            foreach (var itm in packegeDateContract)
                            {
                                ...
                                messageError = itm.Packecge.hosp.НаименованиеПоликлинники.Trim();

                                ControlDantist.ClassErrors.ConvertДоговор convertДоговор = new ClassErrors.ConvertДоговор(itm.Packecge.тДоговор);

                                Т2Договор т2Договор = convertДоговор.Convert();

                                ConvertPerson convertPerson = new ConvertPerson(itm.Packecge.льготник);

                                Т2Льготник person = convertPerson.Convert();

                                // ПРоверим записан ли данный льготник из реестра проектов договров в БД.
                                IValidBD<Т2Льготник> validBPerson = new PersonWriteDBTemp(dc, person);
                                ...
                                IValidBD<Т2Договор> validBDcontract = new ProjectContractTemp(dc, т2Договор);
                                ...
                                {
                                    // Запишем id договора.
                                    т2Договор.id_льготник = validBPerson.Get()?.id_льготник ?? 0;

                                    // Установим временные параметры договора от 1 января 1900 года.
                                    т2Договор.ДатаДоговора = new DateTime(1900, 1, 1);
                                    т2Договор.ДатаАктаВыполненныхРабот = new DateTime(1900, 1, 1);
                                    т2Договор.ДатаРеестра = new DateTime(1900, 1, 1);
                                    т2Договор.ДатаСчётФактура = new DateTime(1900, 1, 1);
                                    т2Договор.датаВозврата = new DateTime(1900, 1, 1);
                                    т2Договор.ДатаЗаписиДоговора = DateTime.Now;
                                    т2Договор.ДатаПроверки = DateTime.Now;

                                    // id поликлинники.
                                    т2Договор.id_поликлинника = itm.Packecge.hosp?.id_поликлинника ?? 0;

                                    dc.Т2Договор.Add(т2Договор);

                                    dc.SaveChanges();

                                    // Сконвертируем список услуг в формат тестовой таблицы.
                                    ConvertУслуги convertУслуги = new ConvertУслуги(itm.Packecge.listUSlug);

                                    // Запишем услуги по договору.
                                    foreach (var uslug in convertУслуги.Convert())
                                    {
                                        uslug.id_договор = т2Договор.id_договор;

                                        dc.Т2УслугиПоДоговору.Add(uslug);
                        ...
                        catch
                        {
                            ...
                            using (TextWriter writer = new StreamWriter(fs))
                            {
                                writer.WriteLine("Ошибка при записи в БД файла - " + messageError);
                            }
                        }
        ...
        private void статистикаКонецГодаToolStripMenuItem_Click(object sender, EventArgs e)
        {
            // Далеко не самое лучшее решение, нужно написать SQL запрос и все заработает.
            // Или еще лучше в едином подключении 
            // И обязательно использовать фабрику.

            IQuery queryДоговраБезакта = new QueryДоговораБезАктовЗаГод();

            // Строка запроса к SQL договора без актов.
            string stringДоговраБезакта = queryДоговраБезакта.Query();

            // Данные с договорами без актов.
            DataTable tabДоговраБезАктов = ТаблицаБД.GetTableSQL(stringДоговраБезакта, "ДоговраБезАктов");

            // SQL запрос договоров с актами выполненных работ.
            IQuery queryДоговраАкт = new QueryДоговораАктЗаГод();
            string stringДоговраАкт = queryДоговраАкт.Query();

            // Данные с договорами с актами выполненных работ.
            DataTable tabДоговраАктов = ТаблицаБД.GetTableSQL(stringДоговраАкт, "ДоговраАктов");

            IQuery queryLk = new QueryСписокЛьготныхКатегорий();
            string stringLk = queryLk.Query();

            // Данные с договорами с актами выполненных работ.
            DataTable tabLk = ТаблицаБД.GetTableSQL(stringLk, "ЛьготныеКатегории");

            List<CountContractForYear> list = new List<CountContractForYear>();

            // Сформируем список и заполним его льготными категориями.
            foreach (DataRow row in tabLk.Rows)
            {
                CountContractForYear dateYear = new CountContractForYear();
                dateYear.ЛьготнаяКатегория = row["ЛьготнаяКатегория"].ToString();
                list.Add(dateYear);
            }

            // Заполним список лимитами на текущий год.
            IQuery queryLimit = new QueryЗаполнитьЛимит(DateTime.Today.Year);
            string queryLimitYear = queryLimit.Query();

            // Данные с договорами с актами выполненных работ.
            DataTable tabLimit = ТаблицаБД.GetTableSQL(queryLimitYear, "Лимит");

            // Заполним данные для отчета годовыми лимитами.
            IFill fillLimit = new FillLimit(list);
            fillLimit.Fill(tabLimit);

            // Заполним данными без актов выполненных работ.
            IFill fillContractNoAct = new FillContractNoAct(list);
            fillContractNoAct.Fill(tabДоговраБезАктов);

            // Заполним данными договоров с актами.
            IFill fillContractAct = new FillContractAct(list);
            fillContractAct.Fill(tabДоговраАктов);
            
            
            //CountContractForYear head = new CountContractForYear();
            //head.КоличествоДоговоровАкт = "Кол-во договоров оплаченных";
            //head.КоличествоДоговоровБезАкта = "Кол-во договоров без акта";
            //head.ЛьготнаяКатегория = "Льготная категория";
            //head.СуммаДоговоров = "Сумма ИТОГО";
            //head.СуммаДоговоровАкт = "Сумма оплаченных договоров";
            //head.СуммаДоговоровБезАкт = "Сумма договоров без акта";
            //head.CountДоговоров = "Всего договоров";
            //list.Add(head);

            //foreach (DataRow row1 in tabДоговраАктов.Rows)
            //{
            ...
            //    //if (iCount2 > 1)
            //    //{
            //        var it = list.Where(w => w.ЛьготнаяКатегория.ToLower().Trim() == item.ЛьготнаяКатегория.ToLower().Trim()).FirstOrDefault();

            //        if (it != null)
            //        {
            //            int sum = Convert.ToInt32(item.КоличествоДоговоровАкт) + Convert.ToInt32(item.КоличествоДоговоровБезАкта);

            //            it.CountДоговоров = sum;
            //            it.СуммаДоговоров = Math.Round(Convert.ToDecimal(item.СуммаДоговоровАкт) + Convert.ToDecimal(item.СуммаДоговоровБезАкт), 4);
            //        }
            //    //}
            ...
            List<CountContractForYear> listDate = new List<CountContractForYear>();

            CountContractForYear l1 = new CountContractForYear();

            var itvvs = list.Where(w => w.ЛьготнаяКатегория.ToLower().Trim() == "Ветеран  военной службы".ToLower().Trim()).FirstOrDefault();

            l1.КоличествоДоговоровБезАкта = itvvs.КоличествоДоговоровБезАкта;
            l1.КоличествоДоговоровАкт = itvvs.КоличествоДоговоровАкт;
            l1.СуммаДоговоровАкт = itvvs.СуммаДоговоровАкт;
            l1.СуммаДоговоровБезАкт = itvvs.СуммаДоговоровБезАкт;

            l1.CountДоговоров = itvvs.КоличествоДоговоровБезАкта + itvvs.КоличествоДоговоровАкт;
            l1.СуммаДоговоров = Math.Round(Convert.ToDecimal(l1.СуммаДоговоровАкт) + Convert.ToDecimal(l1.СуммаДоговоровБезАкт), 4);

            var itvt = list.Where(w => w.ЛьготнаяКатегория.ToLower().Trim() == "Ветеран труда".ToLower().Trim()).FirstOrDefault();

            l1.КоличествоДоговоровБезАкта += itvt.КоличествоДоговоровБезАкта;
            l1.КоличествоДоговоровАкт += itvt.КоличествоДоговоровАкт;
            l1.СуммаДоговоровАкт += itvt.СуммаДоговоровАкт;
            l1.СуммаДоговоровБезАкт += itvt.СуммаДоговоровБезАкт;
            l1.ЛьготнаяКатегория = itvt.ЛьготнаяКатегория + " " + itvvs.ЛьготнаяКатегория;

            int sumCount = itvt.КоличествоДоговоровБезАкта + itvt.КоличествоДоговоровАкт;
            l1.CountДоговоров += sumCount;

            decimal Sum = Math.Round(Convert.ToDecimal(itvt.СуммаДоговоровАкт) + Convert.ToDecimal(itvt.СуммаДоговоровБезАкт), 4);
            l1.СуммаДоговоров += Sum;

            l1.ЛимитГод = itvvs.ЛимитГод + itvt.ЛимитГод;

            listDate.Add(l1);

            listDate.Add(Get("Ветеран труда Саратовской области", list));
            listDate.Add(Get("Реабилитированные лица", list));
            listDate.Add(Get("Труженик тыла", list));

            var asd = list.Sum(w => w.CountДоговоров);

            //// Подсчитаем строку ИТОГО.
            //CountContractForYear count = new CountContractForYear();
            //count.CountДоговоров = list.Sum(w => w.CountДоговоров);
            //count.КоличествоДоговоровАкт  = list.Sum(w => w.КоличествоДоговоровАкт);
            //count.КоличествоДоговоровБезАкта = list.Sum(w => w.КоличествоДоговоровБезАкта);
            //count.СуммаДоговоров = list.Sum(w => w.СуммаДоговоров);
            //count.СуммаДоговоровАкт = list.Sum(w => w.СуммаДоговоровАкт);
            //count.СуммаДоговоровБезАкт = list.Sum(w => w.СуммаДоговоровБезАкт);
            //count.ЛьготнаяКатегория = "ИТОГО";

            // Подсчитаем строку ИТОГО.
            CountContractForYear count = new CountContractForYear();
            count.CountДоговоров = listDate.Sum(w => w.CountДоговоров);
            count.КоличествоДоговоровАкт = listDate.Sum(w => w.КоличествоДоговоровАкт);
            count.КоличествоДоговоровБезАкта = listDate.Sum(w => w.КоличествоДоговоровБезАкта);
            count.СуммаДоговоров = listDate.Sum(w => w.СуммаДоговоров);
            count.СуммаДоговоровАкт = listDate.Sum(w => w.СуммаДоговоровАкт);
            count.СуммаДоговоровБезАкт = listDate.Sum(w => w.СуммаДоговоровБезАкт);
            count.ЛимитГод = listDate.Sum(w => w.ЛимитГод);
            count.ЛьготнаяКатегория = "ИТОГО";

            listDate.Add(count);

            List<ШаблонДляПечати> listPrint = new List<ШаблонДляПечати>();

            ШаблонДляПечати head = new ШаблонДляПечати();
            head.КоличествоДоговоровАкт = "Кол-во договоров оплаченных";
            head.КоличествоДоговоровБезАкта = "Кол-во договоров без акта";
            head.ЛьготнаяКатегория = "Льготная категория";
            head.СуммаДоговоров = "Сумма ИТОГО";
            ...
            head.СуммаДоговоровБезАкт = "Сумма договоров без акта";
            head.CountДоговоров = "Всего договоров";
            head.ЛимитГод = "Лимит на год";
            listPrint.Add(head);

            // Преобразуем все в текстовый формат.
            foreach (var ip in listDate)
            {
                ШаблонДляПечати p = new ШаблонДляПечати();
                p.CountДоговоров = ip.CountДоговоров.ToString();
                p.КоличествоДоговоровАкт = ip.КоличествоДоговоровАкт.ToString();
                p.КоличествоДоговоровБезАкта = ip.КоличествоДоговоровБезАкта.ToString();
                p.ЛьготнаяКатегория = ip.ЛьготнаяКатегория;
                p.СуммаДоговоров = ip.СуммаДоговоров.ToString("C");
                p.СуммаДоговоровАкт = ip.СуммаДоговоровАкт.ToString("C");
                p.СуммаДоговоровБезАкт = ip.СуммаДоговоровБезАкт.ToString("C");
                p.ЛимитГод = ip.ЛимитГод.ToString("C");
            ...
            // Выведим на печать.
            ControlDantist.Reports.IPrintReport report = new ControlDantist.Reports.ОстатокЛимитовОтчет(listPrint);
            report.Print();

        }

        private CountContractForYear Get(string lk, List<CountContractForYear> list)
        {
            CountContractForYear l1 = new CountContractForYear();

            var itvvs = list.Where(w => w.ЛьготнаяКатегория.ToLower().Trim() == lk.ToLower().Trim()).FirstOrDefault();

            l1.ЛьготнаяКатегория = lk;
            ...
            if (itvvs != null)
            {
                ...
                l1.CountДоговоров = itvvs.КоличествоДоговоровБезАкта + itvvs.КоличествоДоговоровАкт;
                l1.СуммаДоговоров = Math.Round(Convert.ToDecimal(l1.СуммаДоговоровАкт) + Convert.ToDecimal(l1.СуммаДоговоровБезАкт), 4);
                l1.ЛимитГод = itvvs.ЛимитГод;
            }
            return l1;

        }

        private void общийЛимитДенежныхСредствЗаГодToolStripMenuItem_Click(object sender, EventArgs e)
        {
